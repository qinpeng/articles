...
对于服务监控，则采用心跳检测的方式，每个服务Container对外暴露一组健康检查的接口，监控系统会定时的巡视所有服务的健康状态，如果由于某种原因被Kill掉，则重启对应Container的并产生告警。

 ![](http://o6wkmqikd.bkt.clouddn.com/maxwon_elb.png)

对于**数据存储分离** 也采用了同样的思路。对于Mongo ,`Pandora` 本身就支持按不同App 数据分治。对于Mysql代理则采用 MaxLeap 研发的 Circe组件，可以实现不同App数据的隔离。使用AWS ELB 解决了Circe的负载均衡与高可用。
